...
#    do
#        echo $(($k+$j))
#        CUDA_VISIBLE_DEVICES=0 python rec_full.py $prefix $(($k+$j)) 1870 1 64 7                 
#    done
# #   wait
# done
...
do
   ...
   do
       echo $(($k+$j))
       CUDA_VISIBLE_DEVICES=0 python rec_crop.py $prefix $(($k+$j)) 1300 1 1024 7                 
   done
   wait
done
...
do
   ...
   do
       echo $(($k+$j))
       python rec_crop2.py $prefix $(($k+$j)) 800 1 512 7 
   done
   wait
done
