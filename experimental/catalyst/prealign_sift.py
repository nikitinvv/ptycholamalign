...
for k in range(ntheta):
    img0[k] = dxchange.read_tiff(data_prefix+'rec_crop/psiangle'+str(nmodes)+str(nscan)+'/r'+str(k)+'.tiff')
...
dxchange.write_tiff_stack(imgres.astype('float32'),data_prefix+'prealign_sift/r.tiff',overwrite=True)
np.save(data_prefix+'/datanpy/shifts_sift.npy',shifts)
